...
impl Store {
    ...
    /// Insert multiple entries inside a single transaction. Much faster than
    /// calling `insert()` in a loop for large batches.
    pub fn insert_batch(&self, entries: &[ClipboardEntry]) -> rusqlite::Result<()> {
    ...
    /// List entries without loading the (potentially large) `content` column.
    /// Use this for populating list views where only metadata is needed.
    pub fn list_metadata(
        ...
        let mut stmt = self.conn.prepare(
            "SELECT id, content_type, preview, hash, byte_size, synced, created_at, source, pinned
             FROM entries ORDER BY pinned DESC, created_at DESC LIMIT ?1 OFFSET ?2",
        )?;
        let rows = stmt.query_map(params![limit as i64, offset as i64], |row| {
            Ok(ClipboardEntry {
                ...
                source: EntrySource::from_str(&row.get::<_, String>(7).unwrap_or_default()),
    ...
    /// Search entries without loading the `content` column.
    pub fn search_metadata(&self, query: &str) -> rusqlite::Result<Vec<ClipboardEntry>> {
        let pattern = format!("%{query}%");
        let mut stmt = self.conn.prepare(
            "SELECT id, content_type, preview, hash, byte_size, synced, created_at, source, pinned
             FROM entries WHERE preview LIKE ?1 OR content LIKE ?1
    ...
    /// Load only the `content` column for a single entry by id.
    pub fn get_content(&self, id: i64) -> rusqlite::Result<Option<String>> {
        let mut stmt = self
            .conn
            .prepare("SELECT content FROM entries WHERE id = ?1")?;
    ...
    /// Check if an entry with the given content hash exists in the database.
    pub fn has_hash(&self, hash: &str) -> bool {
        self.conn
            .query_row(
                "SELECT 1 FROM entries WHERE hash = ?1 LIMIT 1",
                params![hash],
                |_| Ok(()),
            )
            .is_ok()
