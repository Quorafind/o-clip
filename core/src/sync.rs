...
pub enum ClientMessage {
    ...
    /// Request the server to clear all entries and files.
...
pub enum ServerMessage {
    ...
    /// Server tells us all data has been cleared (by another client).
    #[serde(rename = "clear_all")]
    ClearAll,
}
...
/// Commands sent from the main thread to the sync task.
pub enum SyncCommand {
    /// Send a clipboard entry to the server.
    SendEntry(ClipboardEntry),
    /// Request the server to clear all data.
    ClearAll,
}
...
pub enum SyncEvent {
    ...
    /// Server told us all data has been cleared.
    ClearAll,
}
...
pub async fn run_sync(
    ...
    mut outbound_rx: mpsc::UnboundedReceiver<SyncCommand>,
                ...
                loop {
                    tokio::select! {
                        ...
                        Some(cmd) = outbound_rx.recv() => {
                            let msg = match cmd {
                                SyncCommand::SendEntry(entry) => ClientMessage::ClipboardEntry(entry),
                                SyncCommand::ClearAll => ClientMessage::ClearAll,
                            };
...
fn handle_server_message(text: &str, event_tx: &std::sync::mpsc::Sender<SyncEvent>) {
    match serde_json::from_str::<ServerMessage>(text) {
        ...
        Ok(ServerMessage::ClearAll) => {
            tracing::info!("received clear_all from server");
            let _ = event_tx.send(SyncEvent::ClearAll);
        }
